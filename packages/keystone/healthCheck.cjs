...
const getRedisHealthCheck = (clientName = 'healthcheck') => {
    return {
        name: 'redis',
        prepare: () => {
            undefined.redisClient = getRedisClient(clientName);
        },
        run: async () => {
            try {
                const client = undefined.redisClient;
...
const getPostgresHealthCheck = () => {
    return {
        name: 'postgres',
        prepare: ({ keystone }) => {
            undefined.keystone = keystone;
        },
        run: async () => {
            try {
                const { knex } = getDatabaseAdapter(undefined.keystone);
