...
// var createUploadLink = require('apollo-upload-client/createUploadLink.mjs');
var axiosLib = require('axios');
var axiosCookieJarSupportLib = require('axios-cookiejar-support');
...
var loader = require('@open-condo/locales/loader');
var debug = require('debug');

(async function () {
    createUploadLink = await import('apollo-upload-client/createUploadLink.mjs')
})()
...
var createUploadLink__default = /*#__PURE__*/_interopDefaultLegacy(createUploadLink);
var axiosLib__default = /*#__PURE__*/_interopDefaultLegacy(axiosLib);
var axiosCookieJarSupportLib__default = /*#__PURE__*/_interopDefaultLegacy(axiosCookieJarSupportLib);
...
const { flattenDeep, fromPairs, toPairs, get, set, isFunction, isEmpty, template } = lodash__default["default"];
const EXTRA_LOGGING = falsey__default["default"](get(process, 'env.DISABLE_LOGGING'));

const urlParse = urlLib__default["default"].parse;
const axios = axiosLib__default["default"].default;
const axiosCookieJarSupport = axiosCookieJarSupportLib__default["default"].default;

const getRandomString = () => crypto__default["default"].randomBytes(6).hexSlice();
...
const DEFAULT_TEST_USER_IDENTITY = conf__default["default"].DEFAULT_TEST_USER_IDENTITY || 'user@example.com';
const DEFAULT_TEST_USER_SECRET = conf__default["default"].DEFAULT_TEST_USER_SECRET || '1a92b3a07c78';
const DEFAULT_TEST_ADMIN_IDENTITY = conf__default["default"].DEFAULT_TEST_ADMIN_IDENTITY || 'admin@example.com';
const DEFAULT_TEST_ADMIN_SECRET = conf__default["default"].DEFAULT_TEST_ADMIN_SECRET || '3a74b3f07978';
const TESTS_TLS_IGNORE_UNAUTHORIZED = conf__default["default"].TESTS_TLS_IGNORE_UNAUTHORIZED === 'true';
const TESTS_LOG_REQUEST_RESPONSE = conf__default["default"].TESTS_LOG_REQUEST_RESPONSE;
// TODO(pahaz): remove this old consts! we have TESTS_LOG_REQUEST_RESPONSE
const TESTS_LOG_FAKE_CLIENT_RESPONSE_ERRORS = conf__default["default"].TESTS_FAKE_CLIENT_MODE && conf__default["default"].TESTS_LOG_FAKE_CLIENT_RESPONSE_ERRORS;
const TESTS_LOG_REAL_CLIENT_RESPONSE_ERRORS = !conf__default["default"].TESTS_FAKE_CLIENT_MODE && conf__default["default"].TESTS_LOG_REAL_CLIENT_RESPONSE_ERRORS;
const TESTS_REAL_CLIENT_REMOTE_API_URL = conf__default["default"].TESTS_REAL_CLIENT_REMOTE_API_URL || `${conf__default["default"].SERVER_URL}${API_PATH}`;

const SIGNIN_BY_PHONE_AND_PASSWORD_MUTATION = graphqlTag.gql`
...
class UploadingFile {
    constructor (filePath) {
        this.stream = fs__default["default"].createReadStream(filePath);
    }
...
const SIGNIN_BY_EMAIL_MUTATION = graphqlTag.gql`
...
function setFakeClientMode (entryPoint, prepareKeystoneOptions = {}) {
    ...
    if (module.hasOwnProperty('keystone') && module.hasOwnProperty('apps')) {
        ...
        beforeAll(async () => {
            ...
            __expressServer = http__default["default"].createServer(__expressApp).listen(0);
        });
        afterAll(async () => {
...
function initTestExpressApp (name, app, protocol = 'http') {
    ...
    beforeAll(async () => {
        ...
        // nosemgrep: problem-based-packs.insecure-transport.js-node.using-http-server.using-http-server
        __expressTestServers[name].server = await http__default["default"].createServer(app).listen(0);
...
const makeApolloClient = (serverUrl, opts = {}) => {
    ...
    const httpsAgentWithUnauthorizedTls = new https__default["default"].Agent({ rejectUnauthorized: false });

    const apolloLinks = [];
    // Terminating link must be in the end of links chain
    apolloLinks.push(createUploadLink__default["default"]({
    ...
    const client$1 = new client.ApolloClient({
        ...
        link: client.ApolloLink.from(apolloLinks),
    });
    ...
    return {
        ...
        mutate: async (mutation, variables = {}) => {
            return doGqlRequest(client$1.mutate, { mutation, variables }, logRequestResponse)
        },
        query: async (query, variables = {}) => {
            return doGqlRequest(client$1.query, { query, variables }, logRequestResponse)
        },
    }
};
...
const makeClient = async (opts = { generateIP: true, serverUrl: undefined }) => {
    ...
    const customHeaders = {};
    if (opts.generateIP) {
        customHeaders['x-forwarded-for'] = faker.faker.internet.ip();
    }

    if (__expressApp) {
        const port = __expressServer.address().port;
        const protocol = __expressApp instanceof https__default["default"].Server ? 'https' : 'http';
...
const createAxiosClientWithCookie = (options = {}, cookie = '', cookieDomain = '') => {
    const cookies = (cookie) ? cookie.split(';').map(toughCookie.Cookie.parse) : [];
...
class EmptyApp {
    prepareMiddleware () {
        return express__default["default"]()
    }
}

const isPostgres = () => {
    return conf__default["default"].DATABASE_URL.startsWith('postgres')
};

const isMongo = () => {
    return conf__default["default"].DATABASE_URL.startsWith('mongo')
};
...
const expectToThrowGQLError = async (testFunc, errorFields, path = 'obj') => {
    ...
    if (errorFields.messageForUser) {
        const locale = conf__default["default"].DEFAULT_LOCALE;
        const translations = loader.getTranslations(locale);
...
Object.defineProperty(exports, 'gql', {
    enumerable: true,
    get: function () { return graphqlTag.gql; }
});
exports.DATETIME_RE = DATETIME_RE;
exports.DEFAULT_TEST_ADMIN_IDENTITY = DEFAULT_TEST_ADMIN_IDENTITY;
exports.DEFAULT_TEST_ADMIN_SECRET = DEFAULT_TEST_ADMIN_SECRET;
exports.DEFAULT_TEST_USER_IDENTITY = DEFAULT_TEST_USER_IDENTITY;
exports.DEFAULT_TEST_USER_SECRET = DEFAULT_TEST_USER_SECRET;
exports.EmptyApp = EmptyApp;
exports.NUMBER_RE = NUMBER_RE;
exports.UUID_RE = UUID_RE;
exports.UploadingFile = UploadingFile;
exports.catchErrorFrom = catchErrorFrom;
exports.createAxiosClientWithCookie = createAxiosClientWithCookie;
exports.expectToThrowAccessDeniedError = expectToThrowAccessDeniedError;
exports.expectToThrowAccessDeniedErrorToCount = expectToThrowAccessDeniedErrorToCount;
exports.expectToThrowAccessDeniedErrorToObj = expectToThrowAccessDeniedErrorToObj;
exports.expectToThrowAccessDeniedErrorToObjects = expectToThrowAccessDeniedErrorToObjects;
exports.expectToThrowAccessDeniedErrorToResult = expectToThrowAccessDeniedErrorToResult;
exports.expectToThrowAccessDeniedToFieldError = expectToThrowAccessDeniedToFieldError;
exports.expectToThrowAuthenticationError = expectToThrowAuthenticationError;
exports.expectToThrowAuthenticationErrorToObj = expectToThrowAuthenticationErrorToObj;
exports.expectToThrowAuthenticationErrorToObjects = expectToThrowAuthenticationErrorToObjects;
exports.expectToThrowAuthenticationErrorToResult = expectToThrowAuthenticationErrorToResult;
exports.expectToThrowGQLError = expectToThrowGQLError;
exports.expectToThrowGraphQLRequestError = expectToThrowGraphQLRequestError;
exports.expectToThrowInternalError = expectToThrowInternalError;
exports.expectToThrowUniqueConstraintViolationError = expectToThrowUniqueConstraintViolationError;
exports.expectToThrowValidationFailureError = expectToThrowValidationFailureError;
exports.expectValuesOfCommonFields = expectValuesOfCommonFields;
exports.getFeatureFlag = getFeatureFlag;
exports.getRandomString = getRandomString;
exports.getTestExpressApp = getTestExpressApp;
exports.initTestExpressApp = initTestExpressApp;
exports.isMongo = isMongo;
exports.isPostgres = isPostgres;
exports.makeClient = makeClient;
exports.makeLoggedInAdminClient = makeLoggedInAdminClient;
exports.makeLoggedInClient = makeLoggedInClient;
exports.setAllFeatureFlags = setAllFeatureFlags;
exports.setFakeClientMode = setFakeClientMode;
